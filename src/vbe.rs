...
impl TileMapDimensions {
    pub const MAP_32X32: Self = Self {
        width: 32,
        height: 32,
    };
    pub const MAP_64X64: Self = Self {
        width: 64,
        height: 64,
    };
    pub const MAP_128X128: Self = Self {
        width: 128,
        height: 128,
    };
}
...
impl TileMap {
    ...
    pub fn verify_state(&self) -> Result<Hash, VerificationError> {
        ...
        let hash = hash::hash_memory(VirtAddr::new(map_data.as_ptr() as u64), map_data.len());
...
fn fixed_sin(mut x: f32) -> f32 {
    ...
    const SIN_TABLE: [f32; TABLE_SIZE] = [
        0.0, 0.195, 0.383, 0.556, 0.707, 0.831, 0.924, 0.981, 1.0, 0.981, 0.924, 0.831, 0.707,
        0.556, 0.383, 0.195, 0.0, -0.195, -0.383, -0.556, -0.707, -0.831, -0.924, -0.981, -1.0,
        -0.981, -0.924, -0.831, -0.707, -0.556, -0.383, -0.195,
    ];
...
impl VbeDriver {
    ...
    unsafe fn blit_tile_sse2(
        ...
        for ty in 0..height {
            let src_y = if attributes.flip_y {
                height - 1 - ty
            } else {
                ty
            };
            ...
            for tx in 0..width {
                let src_x = if attributes.flip_x {
                    width - 1 - tx
                } else {
                    tx
                };
                ...
                if attributes.enabled {
                    ...
                    if attributes.priority >= current_priority {
                        *dst_row.add(dst_offset) =
                            palette_pixel | ((attributes.priority & 0x3) << 6);
                    }
                }
            }
        }
    ...
    fn cache_tile(&mut self, tile_id: u32, tile: &Tile) -> Result<Hash, VerificationError> {
        let cache_addr = VirtAddr::new(
            self.tile_cache.vram_offset + (tile_id as u64 * (tile.width * tile.height) as u64),
        );
        ...
        let tile_hash = hash::hash_memory(cache_addr, (tile.width * tile.height) as usize);
        ...
        self.tile_cache
            .state_hash
            .store(tile_hash.0, Ordering::SeqCst);
    ...
    pub fn draw_tile(&mut self, tile_id: u32, x: u32, y: u32) -> Result<Hash, VerificationError> {
        let (tile_width, tile_height, tile_data, tile_attributes) = {
            let tile = self
                ...
                .get_tile(tile_id)
                .ok_or(VerificationError::InvalidOperation)?;
            (tile.width, tile.height, tile.data.clone(), tile.attributes)
        };
        ...
        for y in 0..self.height {
            for x in 0..self.width {
                ...
                if x_source < self.width && y_source < self.height {
                    ...
                    unsafe {
                        let src_ptr =
                            (self.framebuffer.as_u64() + source_offset as u64) as *const u8;
    ...
    pub fn hardware_blit(&mut self, request: &BlitRequest) -> Result<(), VerificationError> {
        ...
        match request.operation {
            ...
            BlitOperation::ColorKey(key_color) => unsafe {
                let pitch = self.pitch;
                let width = request.src_region.width;
                let height = request.src_region.height;
            ...
            BlitOperation::Blend(alpha) => unsafe {
                let pitch = self.pitch;
                let width = request.src_region.width;
                let height = request.src_region.height;
        ...
        let blit_region_hash =
            hash::hash_memory(dst_addr, (request.dst_region.height * self.pitch) as usize);
...
impl Verifiable for VbeDriver {
    fn generate_proof(&self, operation: Operation) -> Result<OperationProof, VerificationError> {
        ...
        match operation {
            Operation::Memory {
                ...
                let fb_hash =
                    hash::hash_memory(self.framebuffer, (self.pitch * self.height) as usize);
    ...
    fn verify_proof(&self, proof: &OperationProof) -> Result<bool, VerificationError> {
        let current_hash = hash::hash_memory(self.framebuffer, (self.pitch * self.height) as usize);
